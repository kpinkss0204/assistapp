...
public final class LazyListSnapperLayoutInfo extends SnapperLayoutInfo {
    ...
    /* JADX INFO: renamed from: endContentPadding$delegate, reason: from kotlin metadata */
    ...
    public LazyListSnapperLayoutInfo(LazyListState lazyListState, Function2<? super SnapperLayoutInfo, ? super SnapperLayoutItemInfo, Integer> snapOffsetForItem, int i) {
        ...
        this.endContentPadding = SnapshotStateKt__SnapshotStateKt.mutableStateOf$default(Integer.valueOf(i), null, 2, null);
    ...
    public int determineTargetIndex(float velocity, DecayAnimationSpec<Float> decayAnimationSpec, float maximumFlingDistance) {
        ...
        float fCoerceIn = RangesKt.coerceIn(DecayAnimationSpecKt.calculateTargetValue(decayAnimationSpec, 0.0f, velocity), -maximumFlingDistance, maximumFlingDistance);
        if (velocity < 0.0f) {
        ...
        int iCoerceIn = RangesKt.coerceIn(currentItem.getIndex() + MathKt.roundToInt((((double) fCoerceAtLeast) / d) - (((double) iDistanceToIndexSnap) / d)), 0, getItemCount() - 1);
