...
import com.example.assistapp.features.camera.CameraActivity
import java.util.*
...
class Feature1Activity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    private inner class GestureListener : GestureDetector.SimpleOnGestureListener() {
        ...
        override fun onDoubleTap(e: MotionEvent): Boolean {
            tapCount++
            if (tapCount == 1) {
                binding.featureText.text = "카메라를 실행합니다."
                speak("카메라를 실행합니다.")
                // CameraActivity 실행
                startActivity(Intent(this@Feature1Activity, CameraActivity::class.java))
            } else if (tapCount == 2) {
