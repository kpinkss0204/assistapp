...
public final class SavedStateRegistryImpl {
    ...
    public final Bundle consumeRestoredStateForKey(String key) {
        ...
        Bundle bundleM7369constructorimpl = SavedStateReader.m7369constructorimpl(bundle);
        Bundle bundleM7426getSavedStateimpl = SavedStateReader.m7370containsimpl(bundleM7369constructorimpl, key) ? SavedStateReader.m7426getSavedStateimpl(bundleM7369constructorimpl, key) : null;
        SavedStateWriter.m7491removeimpl(SavedStateWriter.m7455constructorimpl(bundle), key);
        if (SavedStateReader.m7447isEmptyimpl(SavedStateReader.m7369constructorimpl(bundle))) {
            this.restoredState = null;
        }
        return bundleM7426getSavedStateimpl;
    }
    ...
    public final void performRestore$savedstate_release(Bundle savedState) {
        ...
        Bundle bundleM7426getSavedStateimpl = null;
        if (savedState != null) {
            Bundle bundleM7369constructorimpl = SavedStateReader.m7369constructorimpl(savedState);
            if (SavedStateReader.m7370containsimpl(bundleM7369constructorimpl, SAVED_COMPONENTS_KEY)) {
                bundleM7426getSavedStateimpl = SavedStateReader.m7426getSavedStateimpl(bundleM7369constructorimpl, SAVED_COMPONENTS_KEY);
            }
        }
        this.restoredState = bundleM7426getSavedStateimpl;
    ...
    public final void performSave$savedstate_release(Bundle outBundle) {
        ...
        Bundle bundleM7455constructorimpl = SavedStateWriter.m7455constructorimpl(bundleBundleOf);
        Bundle bundle = this.restoredState;
        if (bundle != null) {
            SavedStateWriter.m7459putAllimpl(bundleM7455constructorimpl, bundle);
        }
        synchronized (this.lock) {
            for (Map.Entry entry2 : this.keyToProviders.entrySet()) {
                SavedStateWriter.m7482putSavedStateimpl(bundleM7455constructorimpl, (String) entry2.getKey(), ((SavedStateRegistry.SavedStateProvider) entry2.getValue()).saveState());
            }
            Unit unit = Unit.INSTANCE;
        }
        if (SavedStateReader.m7447isEmptyimpl(SavedStateReader.m7369constructorimpl(bundleBundleOf))) {
            return;
        }
        SavedStateWriter.m7482putSavedStateimpl(SavedStateWriter.m7455constructorimpl(outBundle), SAVED_COMPONENTS_KEY, bundleBundleOf);
    }
}
