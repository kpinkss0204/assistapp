...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m6510getMinimpl(j);
        this.selectionEnd = TextRange.m6509getMaximpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        int iM6510getMinimpl = TextRange.m6510getMinimpl(j);
        int iM6509getMaximpl = TextRange.m6509getMaximpl(j);
        if (iM6510getMinimpl < 0 || iM6510getMinimpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("start (" + iM6510getMinimpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (iM6509getMaximpl < 0 || iM6509getMaximpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("end (" + iM6509getMaximpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (iM6510getMinimpl > iM6509getMaximpl) {
            throw new IllegalArgumentException("Do not set reversed range: " + iM6510getMinimpl + " > " + iM6509getMaximpl);
        }
    }
    ...
    public final TextRange m6671getCompositionMzsxiRA$ui_text_release() {
    ...
    public final long m6672getSelectiond9O1mEE$ui_text_release() {
    ...
    public final void delete$ui_text_release(int start, int end) {
        ...
        long jM6673updateRangeAfterDeletepWDy79M = EditingBufferKt.m6673updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), jTextRange);
        setSelectionStart(TextRange.m6510getMinimpl(jM6673updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m6509getMaximpl(jM6673updateRangeAfterDeletepWDy79M));
        if (hasComposition$ui_text_release()) {
            long jM6673updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m6673updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), jTextRange);
            if (TextRange.m6506getCollapsedimpl(jM6673updateRangeAfterDeletepWDy79M2)) {
                commitComposition$ui_text_release();
            } else {
                this.compositionStart = TextRange.m6510getMinimpl(jM6673updateRangeAfterDeletepWDy79M2);
                this.compositionEnd = TextRange.m6509getMaximpl(jM6673updateRangeAfterDeletepWDy79M2);
            }
        }
    }
