...
public final class AndroidFontLoader implements PlatformFontLoader {
    ...
    public android.graphics.Typeface loadBlocking(Font font) {
        ...
        } else if (FontLoadingStrategy.m6609equalsimpl0(loadingStrategy, FontLoadingStrategy.INSTANCE.m6615getOptionalLocalPKNRLFQ())) {
            try {
                Result.Companion companion = Result.INSTANCE;
                AndroidFontLoader androidFontLoader = this;
                objM7875constructorimpl = Result.m7875constructorimpl(AndroidFontLoader_androidKt.load((ResourceFont) font, this.context));
            } catch (Throwable th) {
                Result.Companion companion2 = Result.INSTANCE;
                objM7875constructorimpl = Result.m7875constructorimpl(ResultKt.createFailure(th));
            }
            typefaceLoad = (android.graphics.Typeface) (Result.m7881isFailureimpl(objM7875constructorimpl) ? null : objM7875constructorimpl);
        } else {
            if (FontLoadingStrategy.m6609equalsimpl0(loadingStrategy, FontLoadingStrategy.INSTANCE.m6613getAsyncPKNRLFQ())) {
                throw new UnsupportedOperationException("Unsupported Async font load path");
            }
            throw new IllegalArgumentException("Unknown loading type " + ((Object) FontLoadingStrategy.m6611toStringimpl(font.getLoadingStrategy())));
        }
