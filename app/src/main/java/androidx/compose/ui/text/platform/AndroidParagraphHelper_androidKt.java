...
public final class AndroidParagraphHelper_androidKt {
    ...
    public static final CharSequence createCharSequence(String str, float f, TextStyle textStyle, List<AnnotatedString.Range<SpanStyle>> list, List<AnnotatedString.Range<Placeholder>> list2, Density density, Function4<? super FontFamily, ? super FontWeight, ? super FontStyle, ? super FontSynthesis, ? extends Typeface> function4, boolean z) {
        ...
        if (z && EmojiCompat.isConfigured()) {
            ...
            EmojiSupportMatch emojiSupportMatchM6364boximpl = (platformStyle == null || (paragraphStyle = platformStyle.getParagraphStyle()) == null) ? null : EmojiSupportMatch.m6364boximpl(paragraphStyle.getEmojiSupportMatch());
            strProcess = EmojiCompat.get().process(str, 0, str.length(), Integer.MAX_VALUE, emojiSupportMatchM6364boximpl == null ? 0 : EmojiSupportMatch.m6367equalsimpl0(emojiSupportMatchM6364boximpl.getValue(), EmojiSupportMatch.INSTANCE.m6371getAll_3YsG6Y()));
            Intrinsics.checkNotNull(strProcess);
        } else {
            strProcess = str;
        }
        if (list.isEmpty() && list2.isEmpty() && Intrinsics.areEqual(textStyle.getTextIndent(), TextIndent.INSTANCE.getNone()) && TextUnitKt.m7238isUnspecifiedR2X_6o(textStyle.m6551getLineHeightXSAIIZE())) {
            return strProcess;
        }
        ...
        if (isIncludeFontPaddingEnabled(textStyle) && textStyle.getLineHeightStyle() == null) {
            SpannableExtensions_androidKt.m6794setLineHeightr9BaKPg(spannable, textStyle.m6551getLineHeightXSAIIZE(), f, density);
            f2 = f;
            density2 = density;
        } else {
            ...
            f2 = f;
            density2 = density;
            SpannableExtensions_androidKt.m6793setLineHeightKmRG4DE(spannable, textStyle.m6551getLineHeightXSAIIZE(), f2, density2, lineHeightStyle);
        }
