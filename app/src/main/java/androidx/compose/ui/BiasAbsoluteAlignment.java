...
public final /* data */ class BiasAbsoluteAlignment implements Alignment {
    ...
    public long mo4173alignKFBX0sM(long size, long space, LayoutDirection layoutDirection) {
        long jIntSize = IntSizeKt.IntSize(IntSize.m7197getWidthimpl(space) - IntSize.m7197getWidthimpl(size), IntSize.m7196getHeightimpl(space) - IntSize.m7196getHeightimpl(size));
        float f = 1;
        return IntOffsetKt.IntOffset(Math.round((IntSize.m7197getWidthimpl(jIntSize) / 2.0f) * (this.horizontalBias + f)), Math.round((IntSize.m7196getHeightimpl(jIntSize) / 2.0f) * (f + this.verticalBias)));
    }
