...
public final class ViewLayer extends View implements OwnedLayer, GraphicLayerInfo {
    ...
    public ViewLayer(AndroidComposeView androidComposeView, DrawChildContainer drawChildContainer, Function2<? super Canvas, ? super GraphicsLayer, Unit> function2, Function0<Unit> function0) {
        ...
        this.mTransformOrigin = TransformOrigin.INSTANCE.m4959getCenterSzJe1aQ();
    ...
    public void updateLayerProperties(ReusableGraphicsLayerScope scope) {
        ...
        if ((mutatedFields$ui_release & 4096) != 0) {
            long jMo4757getTransformOriginSzJe1aQ = scope.getTransformOrigin();
            this.mTransformOrigin = jMo4757getTransformOriginSzJe1aQ;
            setPivotX(TransformOrigin.m4954getPivotFractionXimpl(jMo4757getTransformOriginSzJe1aQ) * getWidth());
            setPivotY(TransformOrigin.m4955getPivotFractionYimpl(this.mTransformOrigin) * getHeight());
        }
        ...
        boolean zM6294updateS_szKao = this.outlineResolver.m6294updateS_szKao(scope.getOutline(), scope.getAlpha(), z3, scope.getShadowElevation(), scope.getSize());
        if (this.outlineResolver.getCacheIsDirty()) {
            updateOutlineResolver();
        }
        boolean z4 = getManualClipPath() != null;
        if (z2 != z4 || (z4 && zM6294updateS_szKao)) {
            invalidate();
        }
        ...
        if (Build.VERSION.SDK_INT >= 28) {
            if ((mutatedFields$ui_release & 64) != 0) {
                ViewLayerVerificationHelper28.INSTANCE.setOutlineAmbientShadowColor(this, ColorKt.m4622toArgb8_81llA(scope.getAmbientShadowColor()));
            }
            if ((mutatedFields$ui_release & 128) != 0) {
                ViewLayerVerificationHelper28.INSTANCE.setOutlineSpotShadowColor(this, ColorKt.m4622toArgb8_81llA(scope.getSpotShadowColor()));
            }
        }
        ...
        if ((mutatedFields$ui_release & 32768) != 0) {
            int iMo4754getCompositingStrategyNrFUSI = scope.getCompositingStrategy();
            if (CompositingStrategy.m4653equalsimpl0(iMo4754getCompositingStrategyNrFUSI, CompositingStrategy.INSTANCE.m4659getOffscreenNrFUSI())) {
                setLayerType(2, null);
            } else if (CompositingStrategy.m4653equalsimpl0(iMo4754getCompositingStrategyNrFUSI, CompositingStrategy.INSTANCE.m4658getModulateAlphaNrFUSI())) {
                setLayerType(0, null);
    ...
    public boolean mo6186isInLayerk4lQ0M(long position) {
        ...
        if (this.clipToBounds) {
            return 0.0f <= fM4327getXimpl && fM4327getXimpl < ((float) getWidth()) && 0.0f <= fM4328getYimpl && fM4328getYimpl < ((float) getHeight());
        }
        if (getClipToOutline()) {
            return this.outlineResolver.m6293isInOutlinek4lQ0M(position);
        }
        return true;
    }
    ...
    public void mo6189resizeozmzZPI(long size) {
        int iM7197getWidthimpl = IntSize.m7197getWidthimpl(size);
        int iM7196getHeightimpl = IntSize.m7196getHeightimpl(size);
        if (iM7197getWidthimpl == getWidth() && iM7196getHeightimpl == getHeight()) {
            return;
        }
        setPivotX(TransformOrigin.m4954getPivotFractionXimpl(this.mTransformOrigin) * iM7197getWidthimpl);
        setPivotY(TransformOrigin.m4955getPivotFractionYimpl(this.mTransformOrigin) * iM7196getHeightimpl);
        updateOutlineResolver();
        layout(getLeft(), getTop(), getLeft() + iM7197getWidthimpl, getTop() + iM7196getHeightimpl);
        resetClipBounds();
        this.matrixCache.invalidate();
    }
    ...
    public void mo6188movegyyYBs(long position) {
        int iM7155getXimpl = IntOffset.m7155getXimpl(position);
        if (iM7155getXimpl != getLeft()) {
            offsetLeftAndRight(iM7155getXimpl - getLeft());
            this.matrixCache.invalidate();
        }
        int iM7156getYimpl = IntOffset.m7156getYimpl(position);
        if (iM7156getYimpl != getTop()) {
            offsetTopAndBottom(iM7156getYimpl - getTop());
            this.matrixCache.invalidate();
        }
    }
    ...
    public long mo6187mapOffset8S9VItk(long point, boolean inverse) {
        if (inverse) {
            float[] fArrM6270calculateInverseMatrixbWbORWo = this.matrixCache.m6270calculateInverseMatrixbWbORWo(this);
            return fArrM6270calculateInverseMatrixbWbORWo != null ? androidx.compose.ui.graphics.Matrix.m4805mapMKHz9U(fArrM6270calculateInverseMatrixbWbORWo, point) : Offset.INSTANCE.m4341getInfiniteF1C5BW0();
        }
        return androidx.compose.ui.graphics.Matrix.m4805mapMKHz9U(this.matrixCache.m6271calculateMatrixGrdbGEg(this), point);
    }
    ...
    public void mapBounds(MutableRect rect, boolean inverse) {
        if (inverse) {
            float[] fArrM6270calculateInverseMatrixbWbORWo = this.matrixCache.m6270calculateInverseMatrixbWbORWo(this);
            if (fArrM6270calculateInverseMatrixbWbORWo != null) {
                androidx.compose.ui.graphics.Matrix.m4807mapimpl(fArrM6270calculateInverseMatrixbWbORWo, rect);
                return;
            } else {
        ...
        androidx.compose.ui.graphics.Matrix.m4807mapimpl(this.matrixCache.m6271calculateMatrixGrdbGEg(this), rect);
    }
    ...
    public void mo6190transform58bKbWc(float[] matrix) {
        androidx.compose.ui.graphics.Matrix.m4816timesAssign58bKbWc(matrix, this.matrixCache.m6271calculateMatrixGrdbGEg(this));
    }
    ...
    public void mo6185inverseTransform58bKbWc(float[] matrix) {
        float[] fArrM6270calculateInverseMatrixbWbORWo = this.matrixCache.m6270calculateInverseMatrixbWbORWo(this);
        if (fArrM6270calculateInverseMatrixbWbORWo != null) {
            androidx.compose.ui.graphics.Matrix.m4816timesAssign58bKbWc(matrix, fArrM6270calculateInverseMatrixbWbORWo);
        }
    }
