...
public final class NestedScrollInteropConnectionKt {
    ...
    public static final long m6290toOffsetUv8p0NA(int[] iArr, long j) {
        float fCoerceAtLeast;
        float fCoerceAtLeast2;
        if (Offset.m4327getXimpl(j) >= 0.0f) {
            fCoerceAtLeast = RangesKt.coerceAtMost(reverseAxis(iArr[0]), Offset.m4327getXimpl(j));
        } else {
            fCoerceAtLeast = RangesKt.coerceAtLeast(reverseAxis(iArr[0]), Offset.m4327getXimpl(j));
        }
        if (Offset.m4328getYimpl(j) >= 0.0f) {
            fCoerceAtLeast2 = RangesKt.coerceAtMost(reverseAxis(iArr[1]), Offset.m4328getYimpl(j));
        } else {
            fCoerceAtLeast2 = RangesKt.coerceAtLeast(reverseAxis(iArr[1]), Offset.m4328getYimpl(j));
        }
    ...
    public static final int m6291toViewTypeGyEprt8(int i) {
        return !NestedScrollSource.m5657equalsimpl0(i, NestedScrollSource.INSTANCE.m5669getUserInputWNlRxjI()) ? 1 : 0;
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* JADX INFO: renamed from: getScrollAxes-k-4lQ0M, reason: not valid java name */
    public static final int m6289getScrollAxesk4lQ0M(long j) {
        int i = Math.abs(Offset.m4327getXimpl(j)) >= 0.5f ? 1 : 0;
        return Math.abs(Offset.m4328getYimpl(j)) >= 0.5f ? i | 2 : i;
    }
