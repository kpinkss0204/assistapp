...
public final class MotionEventAdapter {
    ...
    private final long m5704getComposePointerId_I2yYro(int motionEventPointerId) {
        long jValueAt;
        ...
        return PointerId.m5754constructorimpl(jValueAt);
    }
    ...
    private final PointerInputEventData createPointerInputEventData(PositionCalculator positionCalculator, MotionEvent motionEvent, int index, boolean pressed) {
        long jMo5856localToScreenMKHz9U;
        int iM5854getUnknownT8wyACA;
        long jM4343getZeroF1C5BW0;
        long jM5704getComposePointerId_I2yYro = m5704getComposePointerId_I2yYro(motionEvent.getPointerId(index));
        float pressure = motionEvent.getPressure(index);
        long jOffset = OffsetKt.Offset(motionEvent.getX(index), motionEvent.getY(index));
        long jM4321copydBAh8RU$default = Offset.m4321copydBAh8RU$default(jOffset, 0.0f, 0.0f, 3, null);
        if (index == 0) {
            jMo5856localToScreenMKHz9U = OffsetKt.Offset(motionEvent.getRawX(), motionEvent.getRawY());
            jOffset = positionCalculator.mo5857screenToLocalMKHz9U(jMo5856localToScreenMKHz9U);
        } else if (Build.VERSION.SDK_INT >= 29) {
            jMo5856localToScreenMKHz9U = MotionEventHelper.INSTANCE.m5705toRawOffsetdBAh8RU(motionEvent, index);
            jOffset = positionCalculator.mo5857screenToLocalMKHz9U(jMo5856localToScreenMKHz9U);
        } else {
            jMo5856localToScreenMKHz9U = positionCalculator.mo5856localToScreenMKHz9U(jOffset);
        }
        long j = jMo5856localToScreenMKHz9U;
        long j2 = jOffset;
        int toolType = motionEvent.getToolType(index);
        if (toolType == 0) {
            iM5854getUnknownT8wyACA = PointerType.INSTANCE.m5854getUnknownT8wyACA();
        } else if (toolType == 1) {
            iM5854getUnknownT8wyACA = PointerType.INSTANCE.m5853getTouchT8wyACA();
        } else if (toolType == 2) {
            iM5854getUnknownT8wyACA = PointerType.INSTANCE.m5852getStylusT8wyACA();
        } else if (toolType == 3) {
            iM5854getUnknownT8wyACA = PointerType.INSTANCE.m5851getMouseT8wyACA();
        } else if (toolType == 4) {
            iM5854getUnknownT8wyACA = PointerType.INSTANCE.m5850getEraserT8wyACA();
        } else {
            iM5854getUnknownT8wyACA = PointerType.INSTANCE.m5854getUnknownT8wyACA();
        }
        int i = iM5854getUnknownT8wyACA;
        ...
        if (motionEvent.getActionMasked() == 8) {
            jM4343getZeroF1C5BW0 = OffsetKt.Offset(motionEvent.getAxisValue(10), (-motionEvent.getAxisValue(9)) + 0.0f);
        } else {
            jM4343getZeroF1C5BW0 = Offset.INSTANCE.m4343getZeroF1C5BW0();
        }
        return new PointerInputEventData(jM5704getComposePointerId_I2yYro, motionEvent.getEventTime(), j, j2, pressed, pressure, i, this.activeHoverIds.get(motionEvent.getPointerId(index), false), arrayList, jM4343getZeroF1C5BW0, jM4321copydBAh8RU$default, null);
    }
}
