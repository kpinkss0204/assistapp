...
public final class HitTestResult implements List<Modifier.Node>, KMappedMarker {
    ...
    public final boolean hasHit() {
        long jM6046findBestHitDistanceptXAw2c = m6046findBestHitDistanceptXAw2c();
        return DistanceAndInLayer.m6041getDistanceimpl(jM6046findBestHitDistanceptXAw2c) < 0.0f && DistanceAndInLayer.m6043isInLayerimpl(jM6046findBestHitDistanceptXAw2c);
    }
    ...
    public final boolean isHitInMinimumTouchTargetBetter(float distanceFromEdge, boolean isInLayer) {
        ...
        return DistanceAndInLayer.m6037compareToS_HNhKs(m6046findBestHitDistanceptXAw2c(), HitTestResultKt.DistanceAndInLayer(distanceFromEdge, isInLayer)) > 0;
    }

    /* JADX INFO: renamed from: findBestHitDistance-ptXAw2c, reason: not valid java name */
    private final long m6046findBestHitDistanceptXAw2c() {
        long jDistanceAndInLayer = HitTestResultKt.DistanceAndInLayer(Float.POSITIVE_INFINITY, false);
        int i = this.hitDepth + 1;
        int lastIndex = CollectionsKt.getLastIndex(this);
        if (i <= lastIndex) {
            while (true) {
                long jM6038constructorimpl = DistanceAndInLayer.m6038constructorimpl(this.distanceFromEdgeAndInLayer[i]);
                if (DistanceAndInLayer.m6037compareToS_HNhKs(jM6038constructorimpl, jDistanceAndInLayer) < 0) {
                    jDistanceAndInLayer = jM6038constructorimpl;
                }
                if ((DistanceAndInLayer.m6041getDistanceimpl(jDistanceAndInLayer) < 0.0f && DistanceAndInLayer.m6043isInLayerimpl(jDistanceAndInLayer)) || i == lastIndex) {
                    break;
                }
                i++;
            }
    ...
    public final void speculativeHit(Modifier.Node node, float distanceFromEdge, boolean isInLayer, Function0<Unit> childHitTest) {
        ...
        hitInMinimumTouchTarget(node, distanceFromEdge, isInLayer, childHitTest);
        if (this.hitDepth + 1 < CollectionsKt.getLastIndex(hitTestResult) && DistanceAndInLayer.m6037compareToS_HNhKs(jM6046findBestHitDistanceptXAw2c, m6046findBestHitDistanceptXAw2c()) > 0) {
