...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m4622toArgb8_81llA(colors.get(i).m4578unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i % 2 == 0) {
                fM4328getYimpl = Offset.m4327getXimpl(packedValue);
            } else {
                fM4328getYimpl = Offset.m4328getYimpl(packedValue);
            }
            fArr[i] = fM4328getYimpl;
        }
        return fArr;
    }
}
