...
public class Connector {
    ...
    private Connector(ColorSpace colorSpace, ColorSpace colorSpace2, int i) {
        this(colorSpace, colorSpace2, ColorModel.m4975equalsimpl0(colorSpace.getModel(), ColorModel.INSTANCE.m4982getRgbxdoWZVw()) ? ColorSpaceKt.adapt$default(colorSpace, Illuminant.INSTANCE.getD50(), null, 2, null) : colorSpace, ColorModel.m4975equalsimpl0(colorSpace2.getModel(), ColorModel.INSTANCE.m4982getRgbxdoWZVw()) ? ColorSpaceKt.adapt$default(colorSpace2, Illuminant.INSTANCE.getD50(), null, 2, null) : colorSpace2, i, INSTANCE.m4992computeTransformYBCOT_4(colorSpace, colorSpace2, i), null);
    }
    ...
    public static final class RgbConnector extends Connector {
        ...
        private final float[] m4993computeTransformYBCOT_4(Rgb source, Rgb destination, int intent) {
            ...
            if (RenderIntent.m4998equalsimpl0(intent, RenderIntent.INSTANCE.m5002getAbsoluteuksYyKA())) {
        ...
        public long mo4990transformToColorl2rxGTc$ui_graphics_release(long color) {
            float fM4574getRedimpl = Color.m4574getRedimpl(color);
            float fM4573getGreenimpl = Color.m4573getGreenimpl(color);
            float fM4571getBlueimpl = Color.m4571getBlueimpl(color);
            float fM4570getAlphaimpl = Color.m4570getAlphaimpl(color);
            float fInvoke = (float) this.mSource.getEotfFunc().invoke(fM4574getRedimpl);
            float fInvoke2 = (float) this.mSource.getEotfFunc().invoke(fM4573getGreenimpl);
            float fInvoke3 = (float) this.mSource.getEotfFunc().invoke(fM4571getBlueimpl);
            float[] fArr = this.mTransform;
            return ColorKt.Color((float) this.mDestination.getOetfFunc().invoke((fArr[0] * fInvoke) + (fArr[3] * fInvoke2) + (fArr[6] * fInvoke3)), (float) this.mDestination.getOetfFunc().invoke((fArr[1] * fInvoke) + (fArr[4] * fInvoke2) + (fArr[7] * fInvoke3)), (float) this.mDestination.getOetfFunc().invoke((fArr[2] * fInvoke) + (fArr[5] * fInvoke2) + (fArr[8] * fInvoke3)), fM4570getAlphaimpl, this.mDestination);
        }
    }
    ...
    public static final class Companion {
        ...
        /* JADX INFO: renamed from: computeTransform-YBCOT_4, reason: not valid java name */
        public final float[] m4992computeTransformYBCOT_4(ColorSpace source, ColorSpace destination, int intent) {
            if (!RenderIntent.m4998equalsimpl0(intent, RenderIntent.INSTANCE.m5002getAbsoluteuksYyKA())) {
                return null;
            }
            boolean zM4975equalsimpl0 = ColorModel.m4975equalsimpl0(source.getModel(), ColorModel.INSTANCE.m4982getRgbxdoWZVw());
            boolean zM4975equalsimpl02 = ColorModel.m4975equalsimpl0(destination.getModel(), ColorModel.INSTANCE.m4982getRgbxdoWZVw());
            if (zM4975equalsimpl0 && zM4975equalsimpl02) {
                return null;
            }
            if (!zM4975equalsimpl0 && !zM4975equalsimpl02) {
                return null;
            }
            if (!zM4975equalsimpl0) {
                source = destination;
            }
            Intrinsics.checkNotNull(source, "null cannot be cast to non-null type androidx.compose.ui.graphics.colorspace.Rgb");
            Rgb rgb = (Rgb) source;
            float[] xyz$ui_graphics_release = zM4975equalsimpl0 ? rgb.getWhitePoint().toXyz$ui_graphics_release() : Illuminant.INSTANCE.getD50Xyz$ui_graphics_release();
            float[] xyz$ui_graphics_release2 = zM4975equalsimpl02 ? rgb.getWhitePoint().toXyz$ui_graphics_release() : Illuminant.INSTANCE.getD50Xyz$ui_graphics_release();
        ...
        public final Connector identity$ui_graphics_release(final ColorSpace source) {
            final int iM5004getRelativeuksYyKA = RenderIntent.INSTANCE.m5004getRelativeuksYyKA();
            return new Connector(source, iM5004getRelativeuksYyKA) { // from class: androidx.compose.ui.graphics.colorspace.Connector$Companion$identity$1
                @Override // androidx.compose.ui.graphics.colorspace.Connector
                public float[] transform(float[] v) {
                    return v;
                }
                ...
                {
                    super(source, source, iM5004getRelativeuksYyKA, null);
                }
            };
        }
    }
    ...
    public long mo4990transformToColorl2rxGTc$ui_graphics_release(long color) {
        ...
        float fM4571getBlueimpl = Color.m4571getBlueimpl(color);
        float fM4570getAlphaimpl = Color.m4570getAlphaimpl(color);
        long xy$ui_graphics_release = this.transformSource.toXy$ui_graphics_release(fM4574getRedimpl, fM4573getGreenimpl, fM4571getBlueimpl);
        ...
        float z$ui_graphics_release = this.transformSource.toZ$ui_graphics_release(fM4574getRedimpl, fM4573getGreenimpl, fM4571getBlueimpl);
        float[] fArr = this.transform;
        if (fArr != null) {
        ...
        return this.transformDestination.mo4985xyzaToColorJlNiLsg$ui_graphics_release(f, fIntBitsToFloat2, z$ui_graphics_release, fM4570getAlphaimpl, this.destination);
    }
}
