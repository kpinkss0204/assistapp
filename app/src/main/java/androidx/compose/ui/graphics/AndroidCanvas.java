...
public final class AndroidCanvas implements Canvas {
    ...
    public void mo4423concat58bKbWc(float[] matrix) {
        if (MatrixKt.m4823isIdentity58bKbWc(matrix)) {
            return;
        }
        android.graphics.Matrix matrix2 = new android.graphics.Matrix();
        AndroidMatrixConversions_androidKt.m4438setFromEL8BTi8(matrix2, matrix);
        this.internalCanvas.concat(matrix2);
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* JADX INFO: renamed from: clipRect-N_I0leg, reason: not valid java name */
    public void mo4422clipRectN_I0leg(float left, float top, float right, float bottom, int clipOp) {
        this.internalCanvas.clipRect(left, top, right, bottom, m4431toRegionOp7u2Bmg(clipOp));
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* JADX INFO: renamed from: clipPath-mtrdD-E, reason: not valid java name */
    public void mo4421clipPathmtrdDE(Path path, int clipOp) {
        android.graphics.Canvas canvas = this.internalCanvas;
        if (path instanceof AndroidPath) {
            canvas.clipPath(((AndroidPath) path).getInternalPath(), m4431toRegionOp7u2Bmg(clipOp));
            return;
        }
    ...
    public final Region.Op m4431toRegionOp7u2Bmg(int i) {
        return ClipOp.m4552equalsimpl0(i, ClipOp.INSTANCE.m4556getDifferencertfAjoo()) ? Region.Op.DIFFERENCE : Region.Op.INTERSECT;
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* JADX INFO: renamed from: drawLine-Wko1d7g, reason: not valid java name */
    public void mo4427drawLineWko1d7g(long p1, long p2, Paint paint) {
        this.internalCanvas.drawLine(Offset.m4327getXimpl(p1), Offset.m4328getYimpl(p1), Offset.m4327getXimpl(p2), Offset.m4328getYimpl(p2), paint.getInternalPaint());
    }
    ...
    public void mo4424drawCircle9KIMszo(long center, float radius, Paint paint) {
        this.internalCanvas.drawCircle(Offset.m4327getXimpl(center), Offset.m4328getYimpl(center), radius, paint.getInternalPaint());
    }
    ...
    public void mo4425drawImaged4ec7I(ImageBitmap image, long topLeftOffset, Paint paint) {
        this.internalCanvas.drawBitmap(AndroidImageBitmap_androidKt.asAndroidBitmap(image), Offset.m4327getXimpl(topLeftOffset), Offset.m4328getYimpl(topLeftOffset), paint.getInternalPaint());
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* JADX INFO: renamed from: drawImageRect-HPBpro0, reason: not valid java name */
    public void mo4426drawImageRectHPBpro0(ImageBitmap image, long srcOffset, long srcSize, long dstOffset, long dstSize, Paint paint) {
        ...
        rect.left = IntOffset.m7155getXimpl(srcOffset);
        rect.top = IntOffset.m7156getYimpl(srcOffset);
        rect.right = IntOffset.m7155getXimpl(srcOffset) + IntSize.m7197getWidthimpl(srcSize);
        rect.bottom = IntOffset.m7156getYimpl(srcOffset) + IntSize.m7196getHeightimpl(srcSize);
        Unit unit = Unit.INSTANCE;
        Rect rect2 = this.dstRect;
        Intrinsics.checkNotNull(rect2);
        rect2.left = IntOffset.m7155getXimpl(dstOffset);
        rect2.top = IntOffset.m7156getYimpl(dstOffset);
        rect2.right = IntOffset.m7155getXimpl(dstOffset) + IntSize.m7197getWidthimpl(dstSize);
        rect2.bottom = IntOffset.m7156getYimpl(dstOffset) + IntSize.m7196getHeightimpl(dstSize);
    ...
    public void mo4428drawPointsO7TthRY(int pointMode, List<Offset> points, Paint paint) {
        if (PointMode.m4869equalsimpl0(pointMode, PointMode.INSTANCE.m4873getLinesr_lszbg())) {
            drawLines(points, paint, 2);
        } else if (PointMode.m4869equalsimpl0(pointMode, PointMode.INSTANCE.m4875getPolygonr_lszbg())) {
            drawLines(points, paint, 1);
        } else if (PointMode.m4869equalsimpl0(pointMode, PointMode.INSTANCE.m4874getPointsr_lszbg())) {
    ...
    private final void drawLines(List<Offset> points, Paint paint, int stepBy) {
        if (points.size() >= 2) {
            ...
            while (i < points.size() - 1) {
                ...
                this.internalCanvas.drawLine(Offset.m4327getXimpl(packedValue), Offset.m4328getYimpl(packedValue), Offset.m4327getXimpl(packedValue2), Offset.m4328getYimpl(packedValue2), internalPaint);
                i += stepBy;
            }
        }
    }
    ...
    public void mo4429drawRawPointsO7TthRY(int pointMode, float[] points, Paint paint) {
    ...
    public void mo4430drawVerticesTPEHhCM(Vertices vertices, int blendMode, Paint paint) {
        this.internalCanvas.drawVertices(AndroidVertexMode_androidKt.m4474toAndroidVertexModeJOOmi9M(vertices.getVertexMode()), vertices.getPositions().length, vertices.getPositions(), 0, vertices.getTextureCoordinates(), 0, vertices.getColors(), 0, vertices.getIndices(), 0, vertices.getIndices().length, paint.getInternalPaint());
    }
    ...
    private final void drawPoints(List<Offset> points, Paint paint) {
        ...
        for (int i = 0; i < size; i++) {
            long packedValue = points.get(i).getPackedValue();
            this.internalCanvas.drawPoint(Offset.m4327getXimpl(packedValue), Offset.m4328getYimpl(packedValue), paint.getInternalPaint());
        }
    }
}
