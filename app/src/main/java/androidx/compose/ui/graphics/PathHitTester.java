...
public final class PathHitTester {
    ...
    public final boolean m4852containsk4lQ0M(long position) {
        int i;
        int iLineWinding;
        if (!this.path.isEmpty() && this.bounds.m4353containsk4lQ0M(position)) {
            ...
            if (((IntervalTree) intervalTree).root != ((IntervalTree) intervalTree).terminator) {
                ...
                while (arrayList.size() > 0) {
                    ...
                    if (node.overlaps(fM4328getYimpl, fM4328getYimpl)) {
                        T data = node.getData();
                        ...
                        if (i2 == 1) {
                            iLineWinding = BezierKt.lineWinding(points, fM4327getXimpl, fM4328getYimpl);
                        } else if (i2 == 2) {
                            iLineWinding = BezierKt.quadraticWinding(points, fM4327getXimpl, fM4328getYimpl, fArr, fArr2);
                        } else if (i2 == 3) {
                            iLineWinding = BezierKt.cubicWinding(points, fM4327getXimpl, fM4328getYimpl, fArr, fArr2);
                        }
                        i += iLineWinding;
                    }
                    if (node.getLeft() != ((IntervalTree) intervalTree).terminator && node.getLeft().getMax() >= fM4328getYimpl) {
                        arrayList.add(node.getLeft());
                    }
                    if (node.getRight() != ((IntervalTree) intervalTree).terminator && node.getRight().getMin() <= fM4328getYimpl) {
            ...
            if (PathFillType.m4846equalsimpl0(this.path.mo4459getFillTypeRgk1Os(), PathFillType.INSTANCE.m4850getEvenOddRgk1Os())) {
                i &= 1;
            }
            if (i != 0) {
                return true;
