...
final class ThumbNode extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo375measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j) {
        float thumbDiameter;
        boolean z = (measurable.maxIntrinsicHeight(Constraints.m6980getMaxWidthimpl(j)) == 0 || measurable.maxIntrinsicWidth(Constraints.m6979getMaxHeightimpl(j)) == 0) ? false : true;
        if (this.isPressed) {
            thumbDiameter = SwitchTokens.INSTANCE.m3822getPressedHandleWidthD9Ej5fM();
        } else if (z || this.checked) {
            thumbDiameter = SwitchKt.getThumbDiameter();
        } else {
            thumbDiameter = SwitchKt.getUncheckedThumbDiameter();
        }
        float f = measureScope.mo657toPx0680j_4(thumbDiameter);
        ...
        final Placeable placeableMo5903measureBRTryo0 = measurable.mo5903measureBRTryo0(Constraints.INSTANCE.m6990fixedJhjzzOo(iFloatValue, iFloatValue));
        final float f2 = measureScope.mo657toPx0680j_4(Dp.m7027constructorimpl(Dp.m7027constructorimpl(SwitchKt.SwitchHeight - measureScope.mo653toDpu2uoSUM(f)) / 2.0f));
        float f3 = measureScope.mo657toPx0680j_4(Dp.m7027constructorimpl(Dp.m7027constructorimpl(SwitchKt.SwitchWidth - SwitchKt.getThumbDiameter()) - SwitchKt.ThumbPadding));
        boolean z2 = this.isPressed;
        if (z2 && this.checked) {
            f2 = f3 - measureScope.mo657toPx0680j_4(SwitchTokens.INSTANCE.m3828getTrackOutlineWidthD9Ej5fM());
        } else if (z2 && !this.checked) {
            f2 = measureScope.mo657toPx0680j_4(SwitchTokens.INSTANCE.m3828getTrackOutlineWidthD9Ej5fM());
