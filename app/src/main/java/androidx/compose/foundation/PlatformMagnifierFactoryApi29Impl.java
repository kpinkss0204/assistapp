...
public final class PlatformMagnifierFactoryApi29Impl implements PlatformMagnifierFactory {
    ...
    public PlatformMagnifierImpl mo611createnHHXs2Y(View view, boolean useTextDefault, long size, float cornerRadius, float elevation, boolean clippingEnabled, Density density, float initialZoom) {
        if (useTextDefault) {
            return new PlatformMagnifierImpl(new Magnifier(view));
        }
        long jMo658toSizeXkaWNTQ = density.mo658toSizeXkaWNTQ(size);
        float fMo657toPx0680j_4 = density.mo657toPx0680j_4(cornerRadius);
        float fMo657toPx0680j_42 = density.mo657toPx0680j_4(elevation);
        Magnifier.Builder builder = new Magnifier.Builder(view);
        if (jMo658toSizeXkaWNTQ != InlineClassHelperKt.UnspecifiedPackedFloats) {
            builder.setSize(MathKt.roundToInt(Size.m4396getWidthimpl(jMo658toSizeXkaWNTQ)), MathKt.roundToInt(Size.m4393getHeightimpl(jMo658toSizeXkaWNTQ)));
        }
        if (!Float.isNaN(fMo657toPx0680j_4)) {
            builder.setCornerRadius(fMo657toPx0680j_4);
        }
        if (!Float.isNaN(fMo657toPx0680j_42)) {
            builder.setElevation(fMo657toPx0680j_42);
        }
    ...
    public static final class PlatformMagnifierImpl extends PlatformMagnifierFactoryApi28Impl.PlatformMagnifierImpl {
        ...
        public void mo610updateWko1d7g(long sourceCenter, long magnifierCenter, float zoom) {
            if (!Float.isNaN(zoom)) {
                getMagnifier().setZoom(zoom);
            }
            if (OffsetKt.m4346isSpecifiedk4lQ0M(magnifierCenter)) {
                getMagnifier().show(Offset.m4327getXimpl(sourceCenter), Offset.m4328getYimpl(sourceCenter), Offset.m4327getXimpl(magnifierCenter), Offset.m4328getYimpl(magnifierCenter));
            } else {
                getMagnifier().show(Offset.m4327getXimpl(sourceCenter), Offset.m4328getYimpl(sourceCenter));
            }
        }
    }
}
