...
public final class FlowLayoutBuildingBlocks {
    ...
    public final WrapEllipsisInfo getWrapEllipsisInfo(WrapInfo wrapInfo, boolean hasNext, int lastContentLineIndex, int totalCrossAxisSize, int leftOverMainAxis, int nextIndexInLine) {
        ...
        wrapEllipsisInfoEllipsisInfo$foundation_layout_release.setPlaceEllipsisOnLastContentLine(lastContentLineIndex >= 0 && (nextIndexInLine == 0 || (leftOverMainAxis - IntIntPair.m312getFirstimpl(wrapEllipsisInfoEllipsisInfo$foundation_layout_release.getEllipsisSize()) >= 0 && nextIndexInLine < this.maxItemsInMainAxis)));
    ...
    public final WrapInfo m888getWrapInfoOpUlnko(boolean nextItemHasNext, int nextIndexInLine, long leftOver, IntIntPair nextSize, int lineIndex, int totalCrossAxisSize, int currentLineCrossAxisSize, boolean isWrappingRound, boolean isEllipsisWrap) {
        int i = totalCrossAxisSize + currentLineCrossAxisSize;
        if (nextSize == null) {
            return new WrapInfo(true, true);
        }
        if (this.overflow.getType$foundation_layout_release() != FlowLayoutOverflow.OverflowType.Visible && (lineIndex >= this.maxLines || IntIntPair.m313getSecondimpl(leftOver) - IntIntPair.m313getSecondimpl(nextSize.getPackedValue()) < 0)) {
            return new WrapInfo(true, true);
        }
        if (nextIndexInLine != 0 && (nextIndexInLine >= this.maxItemsInMainAxis || IntIntPair.m312getFirstimpl(leftOver) - IntIntPair.m312getFirstimpl(nextSize.getPackedValue()) < 0)) {
            if (isWrappingRound) {
                return new WrapInfo(true, true);
            }
            return new WrapInfo(true, m888getWrapInfoOpUlnko(nextItemHasNext, 0, IntIntPair.m308constructorimpl(Constraints.m6980getMaxWidthimpl(this.constraints), (IntIntPair.m313getSecondimpl(leftOver) - this.crossAxisSpacing) - currentLineCrossAxisSize), IntIntPair.m305boximpl(IntIntPair.m308constructorimpl(IntIntPair.m312getFirstimpl(nextSize.getPackedValue()) - this.mainAxisSpacing, IntIntPair.m313getSecondimpl(nextSize.getPackedValue()))), lineIndex + 1, i, 0, true, false).getIsLastItemInContainer());
        }
        int iMax = totalCrossAxisSize + Math.max(currentLineCrossAxisSize, IntIntPair.m313getSecondimpl(nextSize.getPackedValue()));
        IntIntPair intIntPairM893ellipsisSizeF35zmw$foundation_layout_release = isEllipsisWrap ? null : this.overflow.m893ellipsisSizeF35zmw$foundation_layout_release(nextItemHasNext, lineIndex, iMax);
        if (intIntPairM893ellipsisSizeF35zmw$foundation_layout_release != null) {
            intIntPairM893ellipsisSizeF35zmw$foundation_layout_release.getPackedValue();
            if (nextIndexInLine + 1 >= this.maxItemsInMainAxis || ((IntIntPair.m312getFirstimpl(leftOver) - IntIntPair.m312getFirstimpl(nextSize.getPackedValue())) - this.mainAxisSpacing) - IntIntPair.m312getFirstimpl(intIntPairM893ellipsisSizeF35zmw$foundation_layout_release.getPackedValue()) < 0) {
                if (isEllipsisWrap) {
                    return new WrapInfo(true, true);
                }
                WrapInfo wrapInfoM888getWrapInfoOpUlnko = m888getWrapInfoOpUlnko(false, 0, IntIntPair.m308constructorimpl(Constraints.m6980getMaxWidthimpl(this.constraints), (IntIntPair.m313getSecondimpl(leftOver) - this.crossAxisSpacing) - Math.max(currentLineCrossAxisSize, IntIntPair.m313getSecondimpl(nextSize.getPackedValue()))), intIntPairM893ellipsisSizeF35zmw$foundation_layout_release, lineIndex + 1, iMax, 0, true, true);
                return new WrapInfo(wrapInfoM888getWrapInfoOpUlnko.getIsLastItemInContainer(), wrapInfoM888getWrapInfoOpUlnko.getIsLastItemInContainer());
            }
        }
