...
final class ParentSizeNode extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo375measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j) {
        ...
        int iM6982getMinWidthimpl = iRound != Integer.MAX_VALUE ? iRound : Constraints.m6982getMinWidthimpl(j);
        int iM6981getMinHeightimpl = iRound2 != Integer.MAX_VALUE ? iRound2 : Constraints.m6981getMinHeightimpl(j);
        if (iRound == Integer.MAX_VALUE) {
            iRound = Constraints.m6980getMaxWidthimpl(j);
        }
        if (iRound2 == Integer.MAX_VALUE) {
            iRound2 = Constraints.m6979getMaxHeightimpl(j);
        }
        final Placeable placeableMo5903measureBRTryo0 = measurable.mo5903measureBRTryo0(ConstraintsKt.Constraints(iM6982getMinWidthimpl, iRound, iM6981getMinHeightimpl, iRound2));
        return MeasureScope.layout$default(measureScope, placeableMo5903measureBRTryo0.getWidth(), placeableMo5903measureBRTryo0.getHeight(), null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.lazy.ParentSizeNode$measure$1
            {
                super(1);
            }
