...
public final class LazyGridAnimateScrollScope implements LazyLayoutAnimateScrollScope {
    ...
    public float calculateDistanceTo(int targetIndex) {
        ...
        if (layoutInfo.getOrientation() == Orientation.Vertical) {
            iM7155getXimpl = IntOffset.m7156getYimpl(lazyGridItemInfo2.getOffset());
        } else {
            iM7155getXimpl = IntOffset.m7155getXimpl(lazyGridItemInfo2.getOffset());
        }
        ...
        while (i < visibleItemsInfo.size()) {
            ...
            if (iIntValue == -1) {
                ...
                while (i < visibleItemsInfo.size() && function1.invoke(Integer.valueOf(i)).intValue() == iIntValue) {
                    if (z) {
                        iM7197getWidthimpl = IntSize.m7196getHeightimpl(visibleItemsInfo.get(i).getSize());
                    } else {
                        iM7197getWidthimpl = IntSize.m7197getWidthimpl(visibleItemsInfo.get(i).getSize());
                    }
                    iMax = Math.max(iMax, iM7197getWidthimpl);
                    i++;
                }
                i2 += iMax;
                i3++;
