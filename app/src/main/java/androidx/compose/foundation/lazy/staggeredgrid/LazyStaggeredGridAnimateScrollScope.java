...
public final class LazyStaggeredGridAnimateScrollScope implements LazyLayoutAnimateScrollScope {
    ...
    public float calculateDistanceTo(int targetIndex) {
        ...
        if (layoutInfo.getOrientation() == Orientation.Vertical) {
            iM7155getXimpl = IntOffset.m7156getYimpl(lazyStaggeredGridItemInfo2.getOffset());
        } else {
            iM7155getXimpl = IntOffset.m7155getXimpl(lazyStaggeredGridItemInfo2.getOffset());
        }
        return iM7155getXimpl;
    }
    ...
    private final int calculateVisibleItemsAverageSize(LazyStaggeredGridLayoutInfo layoutInfo) {
        ...
        for (int i2 = 0; i2 < size; i2++) {
            ...
            if (layoutInfo.getOrientation() == Orientation.Vertical) {
                iM7197getWidthimpl = IntSize.m7196getHeightimpl(lazyStaggeredGridItemInfo.getSize());
            } else {
                iM7197getWidthimpl = IntSize.m7197getWidthimpl(lazyStaggeredGridItemInfo.getSize());
            }
            i += iM7197getWidthimpl;
        }
