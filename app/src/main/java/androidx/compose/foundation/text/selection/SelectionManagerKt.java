...
public final class SelectionManagerKt {
    ...
    public static final Rect getSelectedRegionRect(List<? extends Pair<? extends Selectable, Selection>> list, LayoutCoordinates layoutCoordinates) {
        ...
        while (i2 < size) {
            ...
            if (offset == offset2 || (layoutCoordinates2 = selectableComponent1.getLayoutCoordinates()) == null) {
                ...
                long jMo5912localPositionOfR5De75A = layoutCoordinates.mo5912localPositionOfR5De75A(layoutCoordinates2, jOffset);
                long jMo5912localPositionOfR5De75A2 = layoutCoordinates.mo5912localPositionOfR5De75A(layoutCoordinates2, jOffset2);
                left = Math.min(left, Offset.m4327getXimpl(jMo5912localPositionOfR5De75A));
                top = Math.min(top, Offset.m4328getYimpl(jMo5912localPositionOfR5De75A));
                right = Math.max(right, Offset.m4327getXimpl(jMo5912localPositionOfR5De75A2));
                bottom = Math.max(bottom, Offset.m4328getYimpl(jMo5912localPositionOfR5De75A2));
            }
            i2++;
            size = i;
            c = 0;
    ...
    public static final long m1663calculateSelectionMagnifierCenterAndroidO0kMr_c(SelectionManager selectionManager, long j) {
        ...
        if (i == 1) {
            return m1665getMagnifierCenterJVtK1S4(selectionManager, j, selection.getStart());
        }
        if (i == 2) {
            return m1665getMagnifierCenterJVtK1S4(selectionManager, j, selection.getEnd());
        }
        if (i == 3) {
    ...
    private static final long m1665getMagnifierCenterJVtK1S4(SelectionManager selectionManager, long j, Selection.AnchorInfo anchorInfo) {
        ...
        if (anchorSelectable$foundation_release != null && (containerLayoutCoordinates = selectionManager.getContainerLayoutCoordinates()) != null && (layoutCoordinates = anchorSelectable$foundation_release.getLayoutCoordinates()) != null && (offset = anchorInfo.getOffset()) <= anchorSelectable$foundation_release.getLastVisibleOffset()) {
            Offset offsetM1652getCurrentDragPosition_m7T9E = selectionManager.m1652getCurrentDragPosition_m7T9E();
            Intrinsics.checkNotNull(offsetM1652getCurrentDragPosition_m7T9E);
            float fM4327getXimpl = Offset.m4327getXimpl(layoutCoordinates.mo5912localPositionOfR5De75A(containerLayoutCoordinates, offsetM1652getCurrentDragPosition_m7T9E.getPackedValue()));
            long jMo1613getRangeOfLineContainingjx7JFs = anchorSelectable$foundation_release.mo1613getRangeOfLineContainingjx7JFs(offset);
            if (TextRange.m6506getCollapsedimpl(jMo1613getRangeOfLineContainingjx7JFs)) {
                fCoerceIn = anchorSelectable$foundation_release.getLineLeft(offset);
            } else {
                float lineLeft = anchorSelectable$foundation_release.getLineLeft(TextRange.m6512getStartimpl(jMo1613getRangeOfLineContainingjx7JFs));
                float lineRight = anchorSelectable$foundation_release.getLineRight(TextRange.m6507getEndimpl(jMo1613getRangeOfLineContainingjx7JFs) - 1);
                fCoerceIn = RangesKt.coerceIn(fM4327getXimpl, Math.min(lineLeft, lineRight), Math.max(lineLeft, lineRight));
            }
            if (fCoerceIn == -1.0f) {
                return Offset.INSTANCE.m4342getUnspecifiedF1C5BW0();
            }
            ...
            return containerLayoutCoordinates.mo5912localPositionOfR5De75A(layoutCoordinates, OffsetKt.Offset(fCoerceIn, centerYForOffset));
        }
        return Offset.INSTANCE.m4342getUnspecifiedF1C5BW0();
    }
    ...
    public static final Rect visibleBounds(LayoutCoordinates layoutCoordinates) {
        ...
        return RectKt.m4365Rect0a9Yr6o(layoutCoordinates.mo5920windowToLocalMKHz9U(rectBoundsInWindow.m4362getTopLeftF1C5BW0()), layoutCoordinates.mo5920windowToLocalMKHz9U(rectBoundsInWindow.m4356getBottomRightF1C5BW0()));
    }

    /* JADX INFO: renamed from: containsInclusive-Uv8p0NA, reason: not valid java name */
    public static final boolean m1664containsInclusiveUv8p0NA(Rect rect, long j) {
        float left = rect.getLeft();
        float right = rect.getRight();
