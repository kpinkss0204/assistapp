...
public final class TextFieldTextLayoutModifierNode extends Modifier.Node implements LayoutModifierNode, GlobalPositionAwareModifierNode, CompositionLocalConsumerModifierNode {
    ...
    public MeasureResult mo375measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j) {
        ...
        TextLayoutResult textLayoutResultM1502layoutWithNewMeasureInputshBUhpc = this.textLayoutState.m1502layoutWithNewMeasureInputshBUhpc(measureScope, measureScope.getLayoutDirection(), (FontFamily.Resolver) CompositionLocalConsumerModifierNodeKt.currentValueOf(this, CompositionLocalsKt.getLocalFontFamilyResolver()), j);
        final Placeable placeableMo5903measureBRTryo0 = measurable.mo5903measureBRTryo0(Constraints.INSTANCE.m6989fitPrioritizingWidthZbe2FdA(IntSize.m7197getWidthimpl(textLayoutResultM1502layoutWithNewMeasureInputshBUhpc.getSize()), IntSize.m7197getWidthimpl(textLayoutResultM1502layoutWithNewMeasureInputshBUhpc.getSize()), IntSize.m7196getHeightimpl(textLayoutResultM1502layoutWithNewMeasureInputshBUhpc.getSize()), IntSize.m7196getHeightimpl(textLayoutResultM1502layoutWithNewMeasureInputshBUhpc.getSize())));
        TextLayoutState textLayoutState = this.textLayoutState;
        if (this.singleLine) {
            fM7027constructorimpl = measureScope.mo654toDpu2uoSUM(TextDelegateKt.ceilToIntPx(textLayoutResultM1502layoutWithNewMeasureInputshBUhpc.getLineBottom(0)));
        } else {
            fM7027constructorimpl = Dp.m7027constructorimpl(0);
        }
        textLayoutState.m1503setMinHeightForSingleLineField0680j_4(fM7027constructorimpl);
        LinkedHashMap linkedHashMap = this.baselineCache;
        if (linkedHashMap == null) {
            linkedHashMap = new LinkedHashMap(2);
        }
        linkedHashMap.put(AlignmentLineKt.getFirstBaseline(), Integer.valueOf(Math.round(textLayoutResultM1502layoutWithNewMeasureInputshBUhpc.getFirstBaseline())));
        linkedHashMap.put(AlignmentLineKt.getLastBaseline(), Integer.valueOf(Math.round(textLayoutResultM1502layoutWithNewMeasureInputshBUhpc.getLastBaseline())));
        this.baselineCache = linkedHashMap;
        int iM7197getWidthimpl = IntSize.m7197getWidthimpl(textLayoutResultM1502layoutWithNewMeasureInputshBUhpc.getSize());
        int iM7196getHeightimpl = IntSize.m7196getHeightimpl(textLayoutResultM1502layoutWithNewMeasureInputshBUhpc.getSize());
        Map<AlignmentLine, Integer> map = this.baselineCache;
        Intrinsics.checkNotNull(map);
        return measureScope.layout(iM7197getWidthimpl, iM7196getHeightimpl, map, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.text.input.internal.TextFieldTextLayoutModifierNode$measure$1
            {
                super(1);
            }
