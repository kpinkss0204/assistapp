...
public final class PasswordInputTransformation implements InputTransformation {
    ...
    public void transformInput(TextFieldBuffer textFieldBuffer) {
        if (textFieldBuffer.getChanges().getChangeCount() != 1 || TextRange.m6508getLengthimpl(textFieldBuffer.getChanges().mo1390getRangejx7JFs(0)) != 1 || TextRange.m6508getLengthimpl(textFieldBuffer.getChanges().mo1389getOriginalRangejx7JFs(0)) != 0 || textFieldBuffer.hasSelection()) {
            setRevealCodepointIndex(-1);
            return;
        }
        int iM6510getMinimpl = TextRange.m6510getMinimpl(textFieldBuffer.getChanges().mo1390getRangejx7JFs(0));
        if (getRevealCodepointIndex$foundation_release() != iM6510getMinimpl) {
            this.scheduleHide.invoke();
            setRevealCodepointIndex(iM6510getMinimpl);
        }
    }
