...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m6512getStartimpl(j);
        this.selectionEnd = TextRange.m6507getEndimpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        checkRange(TextRange.m6512getStartimpl(j), TextRange.m6507getEndimpl(j));
    }
    ...
    public final TextRange m1425getCompositionMzsxiRA() {
        if (hasComposition()) {
            return TextRange.m6500boximpl(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd));
        }
        return null;
    }

    /* JADX INFO: renamed from: getSelection-d9O1mEE, reason: not valid java name */
    public final long m1426getSelectiond9O1mEE() {
    ...
    public final void delete(int start, int end) {
        ...
        PartialGapBuffer.replace$default(this.gapBuffer, TextRange.m6510getMinimpl(jTextRange), TextRange.m6509getMaximpl(jTextRange), "", 0, 0, 24, null);
        long jM1428updateRangeAfterDeletepWDy79M = EditingBufferKt.m1428updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), jTextRange);
        setSelectionStart(TextRange.m6512getStartimpl(jM1428updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m6507getEndimpl(jM1428updateRangeAfterDeletepWDy79M));
        if (hasComposition()) {
            long jM1428updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m1428updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), jTextRange);
            if (TextRange.m6506getCollapsedimpl(jM1428updateRangeAfterDeletepWDy79M2)) {
                commitComposition();
            } else {
                this.compositionStart = TextRange.m6510getMinimpl(jM1428updateRangeAfterDeletepWDy79M2);
                this.compositionEnd = TextRange.m6509getMaximpl(jM1428updateRangeAfterDeletepWDy79M2);
            }
        }
        this.highlight = null;
    }
    ...
    public final void m1427setHighlightK7f2yys(int type, int start, int end) {
        if (start >= end) {
            throw new IllegalArgumentException("Do not set reversed or empty range: " + start + " > " + end);
        }
        this.highlight = new Pair<>(TextHighlightType.m1397boximpl(type), TextRange.m6500boximpl(TextRangeKt.TextRange(RangesKt.coerceIn(start, 0, getLength()), RangesKt.coerceIn(end, 0, getLength()))));
    }
