...
public final class MultiWidgetSelectionDelegate implements Selectable {
    ...
    private final synchronized int getLastVisibleOffset(TextLayoutResult textLayoutResult) {
        ...
        if (this._previousTextLayoutResult != textLayoutResult) {
            if (!textLayoutResult.getDidOverflowHeight() || textLayoutResult.getMultiParagraph().getDidExceedMaxLines()) {
                ...
                int iCoerceAtMost = RangesKt.coerceAtMost(textLayoutResult.getLineForVerticalPosition(IntSize.m7196getHeightimpl(textLayoutResult.getSize())), textLayoutResult.getLineCount() - 1);
                while (iCoerceAtMost >= 0 && textLayoutResult.getLineTop(iCoerceAtMost) >= IntSize.m7196getHeightimpl(textLayoutResult.getSize())) {
                    iCoerceAtMost--;
                }
    ...
    public void appendSelectableInfoToBuilder(SelectionLayoutBuilder builder) {
        ...
        long jM4331minusMKHz9U;
        LayoutCoordinates layoutCoordinates = getLayoutCoordinates();
        if (layoutCoordinates == null || (textLayoutResultInvoke = this.layoutResultCallback.invoke()) == null) {
            return;
        }
        long jMo5912localPositionOfR5De75A = builder.getContainerCoordinates().mo5912localPositionOfR5De75A(layoutCoordinates, Offset.INSTANCE.m4343getZeroF1C5BW0());
        long jM4331minusMKHz9U2 = Offset.m4331minusMKHz9U(builder.getCurrentPosition(), jMo5912localPositionOfR5De75A);
        if (OffsetKt.m4348isUnspecifiedk4lQ0M(builder.getPreviousHandlePosition())) {
            jM4331minusMKHz9U = Offset.INSTANCE.m4342getUnspecifiedF1C5BW0();
        } else {
            jM4331minusMKHz9U = Offset.m4331minusMKHz9U(builder.getPreviousHandlePosition(), jMo5912localPositionOfR5De75A);
        }
        MultiWidgetSelectionDelegateKt.m1614appendSelectableInfoParwq6A(builder, textLayoutResultInvoke, jM4331minusMKHz9U2, jM4331minusMKHz9U, getSelectableId());
    }
    ...
    public long mo1612getHandlePositiondBAh8RU(Selection selection, boolean isStartHandle) {
        TextLayoutResult textLayoutResultInvoke;
    ...
    public long mo1613getRangeOfLineContainingjx7JFs(int offset) {
        TextLayoutResult textLayoutResultInvoke = this.layoutResultCallback.invoke();
        if (textLayoutResultInvoke == null) {
