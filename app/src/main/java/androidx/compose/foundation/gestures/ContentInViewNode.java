...
public final class ContentInViewNode extends Modifier.Node implements BringIntoViewResponder, LayoutAwareModifierNode, CompositionLocalConsumerModifierNode {
    ...
    private long viewportSize = IntSize.INSTANCE.m7202getZeroYbymL2g();
    ...
    public Rect calculateRectForParent(Rect localRect) {
        if (IntSize.m7195equalsimpl0(this.viewportSize, IntSize.INSTANCE.m7202getZeroYbymL2g())) {
            throw new IllegalStateException("Expected BringIntoViewRequester to not be used before parents are placed.".toString());
        }
        return m681computeDestinationO0kMr_c(localRect, this.viewportSize);
    }
    ...
    public Object bringChildIntoView(Function0<Rect> function0, Continuation<? super Unit> continuation) {
        ...
        if (rectInvoke == null || m683isMaxVisibleO0kMr_c$default(this, rectInvoke, 0L, 1, null)) {
    ...
    public void mo647onRemeasuredozmzZPI(long size) {
        Rect focusedChildBounds;
        long j = this.viewportSize;
        this.viewportSize = size;
        if (m679compareToTemP2vQ(size, j) < 0 && (focusedChildBounds = getFocusedChildBounds()) != null) {
            Rect rect = this.focusedChildBoundsFromPreviousRemeasure;
            if (rect == null) {
                rect = focusedChildBounds;
            }
            if (!this.isAnimationRunning && !this.trackingFocusedChild && m682isMaxVisibleO0kMr_c(rect, j) && !m682isMaxVisibleO0kMr_c(focusedChildBounds, size)) {
                ...
                if (i == 0) {
                    ...
                    Function1<Float, Unit> function1 = new Function1<Float, Unit>() { // from class: androidx.compose.foundation.gestures.ContentInViewNode.launchAnimation.2.1.1
                        ...
                        public final void invoke(float f) {
                            ...
                            float fM782toFloatk4lQ0M = f2 * scrollingLogic.m782toFloatk4lQ0M(scrollingLogic.m780reverseIfNeededMKHz9U(nestedScrollScope.mo738scrollByOzD1aCk(scrollingLogic.m780reverseIfNeededMKHz9U(scrollingLogic.m783toOffsettuRUvjQ(f2 * f)), NestedScrollSource.INSTANCE.m5669getUserInputWNlRxjI())));
                            if (Math.abs(fM782toFloatk4lQ0M) < Math.abs(f)) {
                                JobKt__JobKt.cancel$default(job, "Scroll animation cancelled because scroll was not consumed (" + fM782toFloatk4lQ0M + " < " + f + ')', null, 2, null);
                            }
                        }
                    };
                    ...
                    if (updatableAnimationState.animateToZero(function1, new Function0<Unit>() { // from class: androidx.compose.foundation.gestures.ContentInViewNode.launchAnimation.2.1.2
                        ...
                        public final void invoke2() {
                            ...
                            while (true) {
                                ...
                                if (!(rectInvoke == null ? true : ContentInViewNode.m683isMaxVisibleO0kMr_c$default(contentInViewNode3, rectInvoke, 0L, 1, null))) {
                                    break;
                                }
                                ...
                                continuation.resumeWith(Result.m7875constructorimpl(unit));
                            }
                            if (contentInViewNode2.trackingFocusedChild) {
                                Rect focusedChildBounds = contentInViewNode2.getFocusedChildBounds();
                                if (focusedChildBounds != null && ContentInViewNode.m683isMaxVisibleO0kMr_c$default(contentInViewNode2, focusedChildBounds, 0L, 1, null)) {
    ...
    public final float calculateScrollDelta(BringIntoViewSpec bringIntoViewSpec) {
        ...
        long jM7209toSizeozmzZPI = IntSizeKt.m7209toSizeozmzZPI(this.viewportSize);
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return bringIntoViewSpec.calculateScrollDistance(rectFindBringIntoViewRequest.getTop(), rectFindBringIntoViewRequest.getBottom() - rectFindBringIntoViewRequest.getTop(), Size.m4393getHeightimpl(jM7209toSizeozmzZPI));
        }
        if (i == 2) {
            return bringIntoViewSpec.calculateScrollDistance(rectFindBringIntoViewRequest.getLeft(), rectFindBringIntoViewRequest.getRight() - rectFindBringIntoViewRequest.getLeft(), Size.m4396getWidthimpl(jM7209toSizeozmzZPI));
        }
    ...
    private final Rect findBringIntoViewRequest() {
        ...
        if (size > 0) {
            ...
            do {
                ...
                if (rectInvoke != null) {
                    if (m680compareToiLBOSCw(rectInvoke.m4360getSizeNHjbRc(), IntSizeKt.m7209toSizeozmzZPI(this.viewportSize)) > 0) {
    ...
    private final Rect m681computeDestinationO0kMr_c(Rect childBounds, long containerSize) {
        return childBounds.m4364translatek4lQ0M(Offset.m4336unaryMinusF1C5BW0(m684relocationOffsetBMxPBkI(childBounds, containerSize)));
    }

    /* JADX INFO: renamed from: isMaxVisible-O0kMr_c$default, reason: not valid java name */
    static /* synthetic */ boolean m683isMaxVisibleO0kMr_c$default(ContentInViewNode contentInViewNode, Rect rect, long j, int i, Object obj) {
        if ((i & 1) != 0) {
            j = contentInViewNode.viewportSize;
        }
        return contentInViewNode.m682isMaxVisibleO0kMr_c(rect, j);
    }

    /* JADX INFO: renamed from: isMaxVisible-O0kMr_c, reason: not valid java name */
    private final boolean m682isMaxVisibleO0kMr_c(Rect rect, long j) {
        long jM684relocationOffsetBMxPBkI = m684relocationOffsetBMxPBkI(rect, j);
        return Math.abs(Offset.m4327getXimpl(jM684relocationOffsetBMxPBkI)) <= 0.5f && Math.abs(Offset.m4328getYimpl(jM684relocationOffsetBMxPBkI)) <= 0.5f;
    }

    /* JADX INFO: renamed from: relocationOffset-BMxPBkI, reason: not valid java name */
    private final long m684relocationOffsetBMxPBkI(Rect childBounds, long containerSize) {
        long jM7209toSizeozmzZPI = IntSizeKt.m7209toSizeozmzZPI(containerSize);
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return OffsetKt.Offset(0.0f, requireBringIntoViewSpec().calculateScrollDistance(childBounds.getTop(), childBounds.getBottom() - childBounds.getTop(), Size.m4393getHeightimpl(jM7209toSizeozmzZPI)));
        }
        if (i == 2) {
            return OffsetKt.Offset(requireBringIntoViewSpec().calculateScrollDistance(childBounds.getLeft(), childBounds.getRight() - childBounds.getLeft(), Size.m4396getWidthimpl(jM7209toSizeozmzZPI)), 0.0f);
        }
        throw new NoWhenBranchMatchedException();
    }

    /* JADX INFO: renamed from: compareTo-TemP2vQ, reason: not valid java name */
    private final int m679compareToTemP2vQ(long j, long j2) {
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return Intrinsics.compare(IntSize.m7196getHeightimpl(j), IntSize.m7196getHeightimpl(j2));
        }
        if (i == 2) {
            return Intrinsics.compare(IntSize.m7197getWidthimpl(j), IntSize.m7197getWidthimpl(j2));
        }
        throw new NoWhenBranchMatchedException();
    }

    /* JADX INFO: renamed from: compareTo-iLBOSCw, reason: not valid java name */
    private final int m680compareToiLBOSCw(long j, long j2) {
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return Float.compare(Size.m4393getHeightimpl(j), Size.m4393getHeightimpl(j2));
        }
        if (i == 2) {
            return Float.compare(Size.m4396getWidthimpl(j), Size.m4396getWidthimpl(j2));
        }
        throw new NoWhenBranchMatchedException();
    }
