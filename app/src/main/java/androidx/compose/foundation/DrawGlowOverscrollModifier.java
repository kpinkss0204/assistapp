...
final class DrawGlowOverscrollModifier extends InspectorValueInfo implements DrawModifier {
    ...
    public void draw(ContentDrawScope contentDrawScope) {
        this.overscrollEffect.m504updateSizeuvyYCjk$foundation_release(contentDrawScope.mo5124getSizeNHjbRc());
        if (Size.m4398isEmptyimpl(contentDrawScope.mo5124getSizeNHjbRc())) {
    ...
    private final boolean drawLeftGlow(DrawScope drawScope, EdgeEffect edgeEffect, Canvas canvas) {
        return m574drawWithRotationAndOffsetubNVwUQ(270.0f, OffsetKt.Offset(-Size.m4393getHeightimpl(drawScope.mo5124getSizeNHjbRc()), drawScope.mo657toPx0680j_4(this.overscrollConfig.getDrawPadding().mo916calculateLeftPaddingu2uoSUM(drawScope.getLayoutDirection()))), edgeEffect, canvas);
    }

    private final boolean drawTopGlow(DrawScope drawScope, EdgeEffect edgeEffect, Canvas canvas) {
        return m574drawWithRotationAndOffsetubNVwUQ(0.0f, OffsetKt.Offset(0.0f, drawScope.mo657toPx0680j_4(this.overscrollConfig.getDrawPadding().getTop())), edgeEffect, canvas);
    }

    private final boolean drawRightGlow(DrawScope drawScope, EdgeEffect edgeEffect, Canvas canvas) {
        return m574drawWithRotationAndOffsetubNVwUQ(90.0f, OffsetKt.Offset(0.0f, (-MathKt.roundToInt(Size.m4396getWidthimpl(drawScope.mo5124getSizeNHjbRc()))) + drawScope.mo657toPx0680j_4(this.overscrollConfig.getDrawPadding().mo917calculateRightPaddingu2uoSUM(drawScope.getLayoutDirection()))), edgeEffect, canvas);
    }

    private final boolean drawBottomGlow(DrawScope drawScope, EdgeEffect edgeEffect, Canvas canvas) {
        return m574drawWithRotationAndOffsetubNVwUQ(180.0f, OffsetKt.Offset(-Size.m4396getWidthimpl(drawScope.mo5124getSizeNHjbRc()), (-Size.m4393getHeightimpl(drawScope.mo5124getSizeNHjbRc())) + drawScope.mo657toPx0680j_4(this.overscrollConfig.getDrawPadding().getBottom())), edgeEffect, canvas);
    }

    /* JADX INFO: renamed from: drawWithRotationAndOffset-ubNVwUQ, reason: not valid java name */
    private final boolean m574drawWithRotationAndOffsetubNVwUQ(float rotationDegrees, long offset, EdgeEffect edgeEffect, Canvas canvas) {
        int iSave = canvas.save();
        canvas.rotate(rotationDegrees);
        canvas.translate(Offset.m4327getXimpl(offset), Offset.m4328getYimpl(offset));
