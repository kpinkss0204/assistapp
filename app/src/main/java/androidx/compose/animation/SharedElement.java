...
public final class SharedElement {
    ...
    public final Rect getTargetBounds() {
        ...
        set_targetBounds(sharedElementInternalState != null ? RectKt.m4367Recttz77jQw(sharedElementInternalState.m390calculateLookaheadOffsetF1C5BW0(), sharedElementInternalState.m391getNonNullLookaheadSizeNHjbRc()) : null);
    ...
    public final void m389onLookaheadResultv_w8tDc(SharedElementInternalState state, long lookaheadSize, long topLeft) {
        if (state.getBoundsAnimation().getTarget()) {
            this.targetBoundsProvider = state;
            Rect rect = get_targetBounds();
            Offset offsetM4316boximpl = rect != null ? Offset.m4316boximpl(rect.m4362getTopLeftF1C5BW0()) : null;
            if (offsetM4316boximpl == null ? false : Offset.m4324equalsimpl0(offsetM4316boximpl.getPackedValue(), topLeft)) {
                Rect rect2 = get_targetBounds();
                Size sizeM4384boximpl = rect2 != null ? Size.m4384boximpl(rect2.m4360getSizeNHjbRc()) : null;
                if (sizeM4384boximpl == null ? false : Size.m4392equalsimpl0(sizeM4384boximpl.getPackedValue(), lookaheadSize)) {
                    return;
                }
            }
            Rect rectM4367Recttz77jQw = RectKt.m4367Recttz77jQw(topLeft, lookaheadSize);
            set_targetBounds(rectM4367Recttz77jQw);
            SnapshotStateList<SharedElementInternalState> snapshotStateList = this.states;
            int size = snapshotStateList.size();
            for (int i = 0; i < size; i++) {
                ...
                boundsAnimation.animate(currentBounds, rectM4367Recttz77jQw);
            }
        }
    }
