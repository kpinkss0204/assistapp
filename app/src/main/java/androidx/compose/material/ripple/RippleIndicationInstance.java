...
public abstract class RippleIndicationInstance implements IndicationInstance {
    ...
    public final void m2047drawStateLayerH2RKhps(DrawScope drawScope, float f, long j) {
        float fM2042getRippleEndRadiuscSwnlzA;
        StateLayer stateLayer = this.stateLayer;
        if (Float.isNaN(f)) {
            fM2042getRippleEndRadiuscSwnlzA = RippleAnimationKt.m2042getRippleEndRadiuscSwnlzA(drawScope, this.bounded, drawScope.mo5124getSizeNHjbRc());
        } else {
            fM2042getRippleEndRadiuscSwnlzA = drawScope.mo657toPx0680j_4(f);
        }
        stateLayer.m2055drawStateLayermxwnekA(drawScope, fM2042getRippleEndRadiuscSwnlzA, j);
    }
}
