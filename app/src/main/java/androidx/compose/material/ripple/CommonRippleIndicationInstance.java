...
final class CommonRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver {
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        float fM2042getRippleEndRadiuscSwnlzA;
        if (Float.isNaN(this.radius)) {
            fM2042getRippleEndRadiuscSwnlzA = RippleAnimationKt.m2042getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo5124getSizeNHjbRc());
        } else {
            fM2042getRippleEndRadiuscSwnlzA = contentDrawScope.mo657toPx0680j_4(this.radius);
        }
        this.targetRadius = fM2042getRippleEndRadiuscSwnlzA;
        long jM4578unboximpl = this.color.getValue().m4578unboximpl();
        contentDrawScope.drawContent();
        ContentDrawScope contentDrawScope2 = contentDrawScope;
        m2047drawStateLayerH2RKhps(contentDrawScope2, this.radius, jM4578unboximpl);
        m2040drawRipples4WTKRHQ(contentDrawScope2, jM4578unboximpl);
    }
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        RippleAnimation rippleAnimation = new RippleAnimation(this.bounded ? Offset.m4316boximpl(interaction.getPressPosition()) : null, this.targetRadius, this.bounded, null);
    ...
    private final void m2040drawRipples4WTKRHQ(DrawScope drawScope, long j) {
        long j2;
        Iterator<Map.Entry<PressInteraction.Press, RippleAnimation>> it = this.ripples.entrySet().iterator();
        while (it.hasNext()) {
            ...
            if (pressedAlpha == 0.0f) {
                ...
                j2 = j;
                value.m2041draw4WTKRHQ(drawScope, Color.m4567copywmQWz5c$default(j2, pressedAlpha, 0.0f, 0.0f, 0.0f, 14, null));
            }
            j = j2;
        }
    }
