...
public final class AndroidRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver, RippleHostKey {
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        int iRoundToInt;
        this.rippleSize = contentDrawScope.mo5124getSizeNHjbRc();
        if (Float.isNaN(this.radius)) {
            iRoundToInt = MathKt.roundToInt(RippleAnimationKt.m2042getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo5124getSizeNHjbRc()));
        } else {
            iRoundToInt = contentDrawScope.mo651roundToPx0680j_4(this.radius);
        }
        ...
        if (rippleHostView != null) {
            rippleHostView.m2046setRippleProperties07v42R4(contentDrawScope.mo5124getSizeNHjbRc(), jM4578unboximpl, pressedAlpha);
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        rippleHostView.m2045addRippleKOepWvA(interaction, this.bounded, this.rippleSize, this.rippleRadius, this.color.getValue().m4578unboximpl(), this.rippleAlpha.getValue().getPressedAlpha(), this.onInvalidateRipple);
