...
public final class SavedStateHandlesProvider implements SavedStateRegistry.SavedStateProvider {
    ...
    public final void performRestore() {
        ...
        if (bundleConsumeRestoredStateForKey != null) {
            SavedStateWriter.m7459putAllimpl(bundleM7455constructorimpl, bundleConsumeRestoredStateForKey);
        }
    ...
    public final Bundle consumeRestoredStateForKey(String key) {
        ...
        performRestore();
        Bundle bundle = this.restoredState;
        if (bundle == null || !SavedStateReader.m7370containsimpl(SavedStateReader.m7369constructorimpl(bundle), key)) {
            return null;
        }
        Bundle bundleM7431getSavedStateOrNullimpl = SavedStateReader.m7431getSavedStateOrNullimpl(SavedStateReader.m7369constructorimpl(bundle), key);
        if (bundleM7431getSavedStateOrNullimpl == null) {
            Map mapEmptyMap = MapsKt.emptyMap();
            if (mapEmptyMap.isEmpty()) {
                pairArr = new Pair[0];
            } else {
            ...
            bundleM7431getSavedStateOrNullimpl = BundleKt.bundleOf((Pair[]) Arrays.copyOf(pairArr, pairArr.length));
            SavedStateWriter.m7455constructorimpl(bundleM7431getSavedStateOrNullimpl);
        }
        ...
        return bundleM7431getSavedStateOrNullimpl;
    }
    ...
    public Bundle saveState() {
        ...
        for (Map.Entry<String, SavedStateHandle> entry2 : getViewModel().getHandles().entrySet()) {
            ...
            if (!SavedStateReader.m7447isEmptyimpl(SavedStateReader.m7369constructorimpl(bundleSaveState))) {
                SavedStateWriter.m7482putSavedStateimpl(bundleM7455constructorimpl, key, bundleSaveState);
            }
        }
        this.restored = false;
